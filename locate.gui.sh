...
var=$(zenity --entry --title='look for what?' --text=locate:)

if [ "$(echo $var |head --bytes 1)" = '/' ];
then
		if [ "$var" = "/refresh" ]; then
				updatedb || killall xmessage &
				xmessage -buttons Abbrechen -center "Refreshing databse... this may take up to ten minutes.  Click Abort to abort the progress." && killall find; killall sort; killall locate;
				exit 1;
		fi
else
